...
import org.digi.lg.neo4j.pojo.model.TermEvent;
import org.digi.lg.neo4j.queries.TermEventQuery;
import org.neo4j.driver.internal.InternalNode;
import org.neo4j.driver.v1.Record;

public class TermEventDaoImpl extends CRUD implements TermEventDao {

	private final BaseDao baseDao;
	private final TermEventQuery termEventQuery;

	protected TermEventDaoImpl(final BaseDao baseDao, final TermEventQuery termEventQuery) {
		super(baseDao);
		this.baseDao = checkNotNull(baseDao);
		this.termEventQuery = checkNotNull(termEventQuery);
	}

	@Override
	public <TRX> TermEvent save(TRX trx, Map<String, Object> params) {
		return new TermEvent(save(trx, SchemaConstants.LABEL_TERM_EVENT, params));
	}

	@Override
	public <TRX> TermEvent getByGuid(TRX trx, String guid) {
		Map<String, Object> bindParams = new HashMap<>(1);
		bindParams.put(BindConstants.PROP_HDMFID_NAME, guid);
		List<Record> records = baseDao.executeQuery(trx, termEventQuery.getByGuid(), bindParams);
		if (records != null && !records.isEmpty()) {
			return TermEvent.rowMapper(((InternalNode) records.get(0).asMap().get("te")).asMap());
		}
		return null;
	}
	...
	public <TRX> Edge createLink(TRX trx, String srcLabel, String destLabel, Relationship relType,
		...
		String query = createEdgeQueryBuilder(termEventQuery.getCreateLink(), handleHyphen(srcLabel),
	...
	public <TRX> Boolean deleteNode(TRX trx, String nodeGuid) {
		...
		baseDao.executeQuery(trx, termEventQuery.getDetachDeleteNode(), bindParams);
		return true;
	}
